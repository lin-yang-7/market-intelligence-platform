...
class AiRankingScorer(Protocol):
    async def score(
        self,
        symbol: str,
        exchange: str,
    ...
    async def score(
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
                response = await client.post(f"{self.base_url}/v1/ai/predict", json=payload)
